...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node *copy_nodes_impl(Node *node) {
    ...
    node_copy->datum = node->datum;
    node_copy->left = copy_nodes_impl(node->left);
    node_copy->right = copy_nodes_impl(node->right);
    return node_copy;
  }
  ...
  static Node * insert_impl(Node *node, const T &item, Compare less) {
    if(!node) {
      ...
      insert_node->datum = item;
      insert_node->left = nullptr;
      insert_node->right = nullptr;
      return insert_node;
    }
    else if(!node->left && less(item, node->datum)) {
      Node* insert_node = new Node;
      insert_node->datum = item;
      insert_node->left = nullptr;
      insert_node->right = nullptr;
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    ...
    if(node->left) {
      if(less((node->left)->datum, node->datum)) {check_left = 
      check_sorting_invariant_impl(node->left, less);}
      else {check_left = false;}
    }
    if(node->right) {
      if(less(node->datum, (node->right)->datum)) {check_right = 
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    ...
    if(less(val, node->datum)) {
      if(!node->left) {return node;}
      if(less(val, (node->left)->datum) || (node->left)->right) 
      {return min_greater_than_impl(node->left, val, less);}
      else {return node;}
    }
    else {
      if(!node->right) {
        if(less(val, node->datum)) {return node;}
        else {return nullptr;}
      }
